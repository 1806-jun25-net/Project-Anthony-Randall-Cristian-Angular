...
export class ZVRPubapiService {
  ...
  getAllLocationsById(id: string,success, failure) {

    let url = 'https://project2zvrapi.azurewebsites.net/api/locations/' + id;
    let request = this.httpClient.get(url);
    let promise = request.toPromise();
  ...
  getAllLocationsInventory(success, failure) {

    let url = 'https://project2zvrapi.azurewebsites.net/api/inventoryHasLocation'
    let request = this.httpClient.get(url);
    let promise = request.toPromise();

    promise.then((res) => { console.log("gathered locationsHasInventory inventory from database"); success(res); }, (res) => console.log("failed to gather locations from database"));
  }

  getAllInventory(success, failure) {

    let url = 'https://project2zvrapi.azurewebsites.net/api/Inventory'
    let request = this.httpClient.get(url);
    let promise = request.toPromise();

    promise.then((res) => { console.log("gathered inventory inventory from database"); success(res); }, (res) => console.log("failed to gather locations from database"));
  }
